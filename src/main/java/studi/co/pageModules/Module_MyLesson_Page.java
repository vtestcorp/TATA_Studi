...
//			test.log(Status.INFO, "User unable to navigate to MyLessons Page");
	...
	//	scrollTo1("Upcoming");
		...
		System.out.println(topicsInUpcoming.size());
		for(MobileElement topic : topicsInUpcoming) {
...
//		omp.practice.click();
		...
		String colour1=colour.getAttribute("class");
		...
		String colour3=colour2.getAttribute("class");
	...
	public void toVerify_Lesson_Card_Content() throws Exception {
		...
		for(MobileElement list : todayList) {
			...
			Boolean lessonStatus1=lessonStatus.isDisplayed();
			if(lessonStatus1) {
	...
	//			System.out.println(lessonStatus.getText());
			...
			Boolean lessonType=lessonStatus.isDisplayed();
			if(lessonType) {
				System.out.println("lesson type is Displayed");
				System.out.println(typeOfLesson.getText());
			}
		}
		...
		for(MobileElement list : upcomingList) {
			...
			Boolean lessonStatus1=(lessonStatus.isDisplayed() || lessonStatus12.isDisplayed());
			if(lessonStatus1) {
				System.out.println("lesson status is Displayed");
	//			System.out.println(lessonStatus.getText());
			}
...
//		omp.topic.click();
	...
	public void toVerify_Schedule_List_Of_Plan() throws Exception {
		...
		System.out.println(listOfLessons.size());
...
//			Keyword.scrollToEnd();
			for(MobileElement lesson : listOfLessons) {
	...
	public void toVerify_Schedule_List_Of_Plan_For_Other_Weeks() throws Exception {
		...
		for(int i= currentweek;i<=totalWeeks;i++) {
			...
			String currentWeek1=getBetweenStrings(totalweeks1, "Week", "of").trim();
	...
	//	String [] contents= {"SUBJECTS","LESSON TYPE","LESSON STATUS"};
		...
		System.out.println(lessonStatus.size());
			for(int j=1;j<=lessonStatus.size();j++) {
	...
	public void toVerify_Various_Filter_Options_On_Schedule_List_Should_Be_Functional() throws Exception {
		...
		System.out.println(lessonStatus1.size());
		...
		System.out.println(lessonStatus.size());
			...
			System.out.println(lessonStatus12.size());
				for(int j=4;j<=lessonStatus12.size();j++) {
			...
			System.out.println(lessonStatus23.size());
				for(int j=1;j<=lessonStatus23.size();j++) {
				...
				System.out.println(lessonType.size());
				for (MobileElement mobileElement : lessonType) {
...
//			omp.backIcon.click();
			...
			WebElement date=getDriver().findElementById("com.tce.studi:id/tv_week");
	...
	public void toVerify_HighLighted_Lesson() throws Exception {
		...
		System.out.println(highLightedLesson.getAttribute("resource-id"));
		Boolean highLightedCard=highLightedLesson.isDisplayed();
		if(highLightedCard) {
		...
		String lessonType=highLightedLesson.findElement(By.xpath("//android.view.ViewGroup/android.widget.TextView[1]")).getText();
		...
		String todaysTasks=omp.taskToBeCompletedToday.getText();
			...
			for(int i=0;i<tasks;i++) {
				...
				//	String lessonType1=highLightedLesson.findElement(By.xpath("//android.view.ViewGroup/android.widget.TextView[1]")).getText();
					String lessonType1=highLightedLesson.findElement(By.id("com.tce.studi:id/tv_chapter_type")).getText();
...
//		}
	...
	public void toVerify_Manage_And_Create_Plan_Is_Functional() throws Exception {
		...
		for(MobileElement plan : otherPlans){
	...
	public void toVerify_Locked_Lessons_Functionality() throws Exception {
		...
		lessonStatus.addAll(lessons1);
		lessonStatus.addAll(lessons2);
//		List<MobileElement> lessons1=getDriver().findElementsByXPath("/hierarchy/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.FrameLayout/android.widget.LinearLayout/android.widget.FrameLayout/android.view.ViewGroup/android.view.ViewGroup/android.widget.ScrollView/android.view.ViewGroup/android.widget.LinearLayout/android.widget.LinearLayout/android.widget.LinearLayout/android.view.ViewGroup/androidx.recyclerview.widget.RecyclerView/android.view.ViewGroup");
		System.out.println(lessonStatus.size()+"size");
		
	//	int j=1;
	//	for(MobileElement lesson : lessons1) {
...
//		System.out.println(web.getAttribute("resource-id")+"--------");
			if(web.getAttribute("resource-id").equals("com.tce.studi:id/locked")) {
