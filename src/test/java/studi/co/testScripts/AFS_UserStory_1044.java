...
public class AFS_UserStory_1044 extends BaseClass {
	...
	@Test(enabled = false, groups = "NewUser") // only for new user(No previously active study plan)
	...
	@Test(enabled = false, groups = "ExistingUser") // Validate only if user has created a plan previously
	...
	@Test(enabled = false, groups = "ExistingUser") // Validate only if user has created a plan and the plan is still
													// active
	...
	@Test(enabled = false, groups = "ExistingUser") // Validate only if user has created a plan and the plan is paused
	...
	@Test(enabled = false, groups = "ExistingUser")
